...
// infra/lambda-src/webinar/submit.ts
var submit_exports = {};
...
var STATE_NAMES = US_STATES.map((s) => s.name);
...
// shared/schema.ts
var asTuple = (a) => a;
var emptyToUndef = (schema) => external_exports.preprocess((v) => v === "" || v == null ? void 0 : v, schema.optional());
...
var registrationFormSchema = external_exports.object({
  firstName: external_exports.string().trim().min(1, "Please enter your first name").max(80),
  lastName: external_exports.string().trim().min(1, "Please enter your last name").max(80),
  email: external_exports.string().trim().toLowerCase().email("Enter a valid email address").max(254),
  role: external_exports.enum(asTuple(ROLES), { errorMap: () => ({ message: "Select your role" }) }),
  parentOrg: external_exports.enum(PARENT_ORG_IDS, { errorMap: () => ({ message: "Select a parent organization" }) }),
  systemName: external_exports.string().trim().min(1, "Enter your system / title name").max(160),
  level: external_exports.enum(asTuple(LEVELS), { errorMap: () => ({ message: "Select a level" }) }),
  state: external_exports.enum(STATE_NAMES, { errorMap: () => ({ message: "Select a state" }) }),
  consentMarketing: external_exports.boolean(),
  // Optional
...
var utmSchema = external_exports.object({
  ...
  medium: external_exports.string().max(200).optional(),
  campaign: external_exports.string().max(200).optional(),
  term: external_exports.string().max(200).optional(),
  content: external_exports.string().max(200).optional()
}).partial();
var submitEnvelopeSchema = registrationFormSchema.extend({
  // Honeypot: must be empty. Real users never see it.
  company: external_exports.string().max(0, "bot").optional().or(external_exports.literal("")),
  // Ms since form render — enforced >= 3000ms in the handler.
  formRenderedAt: external_exports.number().int().nonnegative().optional(),
  utm: utmSchema.optional(),
  referrer: emptyToUndef(external_exports.string().max(2048))
});
...
function normalize(input) {
  return String(input ?? "").toLowerCase().replace(/[.'’,&]/g, "").replace(/[-/]/g, " ").split(/\s+/).filter((w) => w.length > 0 && !NOISE_WORDS.has(w)).join(" ").trim();
}
var HONORIFICS = /* @__PURE__ */ new Set(["miss", "mrs", "ms", "mr"]);
function isHonorificSwap(a, b) {
  const strip = (s) => s.split(" ").filter((w) => !HONORIFICS.has(w)).join(" ");
  const honos = (s) => s.split(" ").filter((w) => HONORIFICS.has(w)).sort().join(" ");
  return strip(a) === strip(b) && honos(a) !== honos(b);
}

// shared/jaro-winkler.ts
function jaro(s1, s2) {
  if (s1 === s2) return 1;
...
// shared/match.ts
var AUTO_THRESHOLD = 0.92;
var REVIEW_THRESHOLD = 0.75;
function classifyScore(score) {
  if (score >= AUTO_THRESHOLD) return "auto";
  if (score >= REVIEW_THRESHOLD) return "review";
  return "new";
}
function matchSystem(rawInput, candidates) {
...
function newJoinToken() {
  return (0, import_crypto.randomBytes)(24).toString("base64url");
}
function resolveJoinToken(existing) {
...
async function querySystems(parentOrgId) {
  const out = [];
...
async function putRegistration(item) {
  await ddb.send(new import_lib_dynamodb.PutCommand({ TableName: TABLE, Item: item }));
}
async function isRateLimited(ip, limitPerMin = 10) {
  if (!ip) return false;
...
// infra/lambda-src/webinar/submit.ts
var MIN_SUBMIT_MS = 3e3;
var handler = async (event) => {
  const now = Date.now();
  const sourceIp = event.requestContext?.http?.sourceIp || (event.headers?.["x-forwarded-for"] || "").split(",")[0].trim() || null;
  ...
  const item = {
    ...
    systemNameRaw: d.systemName,
    // verbatim — never overwritten with canonical
    systemId: systemId ?? void 0,
    matchScore,
    matchStatus,
    level: d.level,
    state: d.state,
